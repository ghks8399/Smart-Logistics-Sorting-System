"""
Vision Agent (자가 트리거 + 4모서리 진입 트리거) — v6.1 fragile 마크 검출 추가

v6.1 변경:
  - fragile 마크(빨간 깨진 유리잔) OpenCV 검출 추가 (vision/fragile_detector.py)
  - QR 성공 시: QR의 fragile 정보 ↔ 실물 마크 검출 결과 교차검증
  - QR 실패→OCR 경로: 마크 검출로 fragile 판정, sort_code에 _FRAGILE 반영
  - STRICT_FRAGILE_CHECK 플래그: True면 fragile 정보-마크 불일치 시 DEFECT
  - 스트림 화면에 마크 검출 시 빨간 표시 (FRAGILE MARK DETECTED)
  - WPF payload에는 fragile_mark 미포함 (내부 검증용)

★ v6.1.1 (현재):
  - STRICT_FRAGILE_CHECK = False 로 변경.
    불량 처리 조건은 (1) QR/OCR 둘 다 인식 실패 또는 (2) QR↔OCR 불일치 뿐.
    fragile 마크 불일치는 DEFECT 로 빼지 않고 경고 로그만 남기고 QR 기준으로 분류.
    (비닐/정상 박스가 마크 오검출로 DEFECT 빠지던 문제 해결)
...
from vision.fragile_detector import detect_fragile_mark
...
# fragile 교차검증 모드
#   True  : QR fragile 정보와 실물 마크가 불일치하면 DEFECT(mismatch) 처리
#   False : 불일치는 경고 로그만 남기고, 분류는 QR/마크 기준으로 진행
#
# ★ False 로 둔다.
#   불량(DEFECT) 처리 조건은 다음 두 가지 뿐:
#     (1) QR/OCR 둘 다 인식 실패
#     (2) QR ↔ OCR 불일치
#   fragile 마크는 분류 정보일 뿐, 마크가 안 맞아도 DEFECT 로 빼지 않는다.
#   (비닐/정상 박스가 마크 오검출로 DEFECT 빠지던 문제 해결)
STRICT_FRAGILE_CHECK = False

# 화면 표시 지속 시간(초) — 마크 검출 후 스트림에 표시 유지
FRAGILE_DISPLAY_SEC = 5.0
...
# 화면 표시용 — 마크 검출 결과 (스캔 시점에 갱신)
last_fragile_info = {"detected": False, "ts": 0.0}
...
def draw_overlay(frame, state_label, debug_corners=None, debug_text=None, box_center=None):
    ...
    # fragile 마크 검출 결과 표시 (검출 후 FRAGILE_DISPLAY_SEC 동안)
    if last_fragile_info["detected"] and \
       (time.time() - last_fragile_info["ts"] < FRAGILE_DISPLAY_SEC):
        cv2.putText(out, "FRAGILE MARK DETECTED", (10, h - 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 3)
        cv2.rectangle(out, (5, 5), (w - 5, h - 5), (0, 0, 255), 6)
...
class ScanController:
    ...
    def _scan_worker(self):
        ...
        # ── fragile 마크 검출 (박스 ROI에서, 1~5ms) ──
        mark_detected, red_px = detect_fragile_mark(box_roi, debug=True)
        print(f"[VISION] 파손주의 마크: {'있음' if mark_detected else '없음'} (red_px={red_px})")

        # 화면 표시용 저장
        global last_fragile_info
        last_fragile_info = {"detected": mark_detected, "ts": time.time()}

        self._finalize_and_publish(qr_result, ocr, mark_detected)
        self.absence_count = 0
        self._set_state(self.COOLDOWN)
        print("[VISION] 스캔 종료 → COOLDOWN (ROI 비워지길 대기)\n")

    def _finalize_and_publish(self, qr, ocr, mark_detected=False):
        """
        QR/OCR 결과 + fragile 마크 결합 후 발행.

        ★ 불량(DEFECT) 처리 조건은 다음 두 가지 뿐:
            (1) QR/OCR 둘 다 인식 실패  → publish_scan_failed
            (2) QR ↔ OCR 불일치         → publish_mismatch
          fragile 마크 불일치는 STRICT_FRAGILE_CHECK=False 이므로
          경고 로그만 남기고 QR 기준으로 정상 분류한다.

        - QR 성공 + OCR 성공: 교차검증 통과 시 success, 실패 시 mismatch(불량)
        - QR 성공 + OCR 없음: success (STRICT_CROSSCHECK=False 기준)
        - QR 실패 + OCR 성공: OCR 유효 시 success
        - QR 실패 + OCR 실패: fail(불량)
        """
        sid = self.scan_id
        if qr and ocr:
            ok, reason = cross_check(qr, ocr)
            if ok:
                # fragile 마크는 분류 정보일 뿐 — 불일치해도 경고만, DEFECT 안 함
                qr_fragile = bool(qr.get("fragile"))
                if qr_fragile != mark_detected:
                    fr_reason = ("FRAGILE_MARK_MISSING" if qr_fragile
                                 else "FRAGILE_DATA_MISSING")
                    if STRICT_FRAGILE_CHECK:
                        print(f"[VISION] fragile 불일치 → DEFECT: {fr_reason} "
                              f"(qr={qr_fragile}, mark={mark_detected})")
                        self.agent.publish_mismatch(sid, qr, ocr, fr_reason)
                        return
                    else:
                        print(f"[VISION] fragile 불일치 경고: {fr_reason} "
                              f"(qr={qr_fragile}, mark={mark_detected}) — QR 기준 분류 진행")
            ...
            if STRICT_CROSSCHECK:
                ...
                if qr_fragile != mark_detected:
                    ...
                    if STRICT_FRAGILE_CHECK:
                        print(f"[VISION] fragile 불일치 → DEFECT: {fr_reason} "
                              f"(qr={qr_fragile}, mark={mark_detected})")
                        self.agent.publish_mismatch(sid, qr, None, fr_reason)
                        return
                    else:
                        print(f"[VISION] fragile 불일치 경고: {fr_reason} "
                              f"(qr={qr_fragile}, mark={mark_detected}) — QR 기준 분류 진행")
                self.agent.publish_qr_success(sid, qr, None,
                                              "OCR_UNREADABLE_IGNORED",
                                              mark_detected=mark_detected)
        elif not qr and ocr:
            ...
            if has_invoice or has_region:
                self.agent.publish_ocr_success(sid, ocr, mark_detected=mark_detected)
            else:
...
class VisionAgent(AgentBase):
    ...
    def publish_qr_success(self, scan_id, qr, ocr, verify_reason, mark_detected=False):
        ...
        if ocr:
            print(f"[VISION] QR+OCR 교차검증 성공: {sort_code} ({verify_reason}) "
                  f"fragile(qr={qr.get('fragile')}, mark={mark_detected})")
        else:
            print(f"[VISION] QR 단독 성공: {sort_code} ({verify_reason}) "
                  f"fragile(qr={qr.get('fragile')}, mark={mark_detected})")
        self._safe_publish(VISION_SCAN_RESULT, payload, "qr_success")

    def publish_ocr_success(self, scan_id, ocr, mark_detected=False):
        # OCR 경로: 최종 sort_code/fragile 판정은 Flask가 manifest(DB)와 대조해서 결정.
        # Vision은 _FRAGILE을 미리 붙이지 않고 마크 검출 결과(fragile_mark)만 전달.
        payload = {
            "scan_id":        scan_id,
            "package_id":     None,
            ...
            "fragile_mark":   mark_detected,   # Flask 대조용 (WPF는 무시)
            "scan_method":    "OCR",
        ...
        print(f"[VISION] OCR 폴백 성공: invoice={ocr['invoice_no']} "
              f"region={ocr['region_code']} name={ocr['name']} mark={mark_detected}")
        self._safe_publish(VISION_SCAN_RESULT, payload, "ocr_success")

    def publish_mismatch(self, scan_id, qr, ocr, reason):
...
if __name__ == "__main__":
    ...
    print(f"[STARTUP] STRICT_FRAGILE_CHECK = {STRICT_FRAGILE_CHECK} "
          f"(False=마크 불일치는 경고만, DEFECT 안 함)")
